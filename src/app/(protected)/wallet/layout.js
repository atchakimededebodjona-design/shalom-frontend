...
const TABS = [
  { href: '/wallet', label: 'Aperçu' },
  { href: '/wallet/transactions', label: 'Mouvements' },
  { href: '/wallet/categories', label: 'Catégories' },
];
  ...
  return (
    <div className={`container animate-fade-in ${styles.theme}`} style={{ padding: 'var(--spacing-xl) 0' }}>
      ...
      <h1 className="text-primary flex items-center gap-sm mb-md">
        <WalletMinimal size={26} /> Portefeuille
      </h1>
      <nav className={styles.tabs}>
        {TABS.map((t) => {
          const active = t.href === '/wallet' ? pathname === '/wallet' : pathname.startsWith(t.href);
          return (
