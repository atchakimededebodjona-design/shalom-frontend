...
import { CategoryForm } from '../../../../components/wallet/CategoryForm';
        ...
        <div className={styles.list}>
          {cats.map((c) => (
            <div key={c.id} className={styles.txRow}>
              <span className={styles.txIcon} style={{ background: c.color || 'var(--bg-color)', color: '#fff' }}>{c.icon || '🏷️'}</span>
              <div className={styles.txMain}>
                <div className={styles.txTitle}>{c.name}</div>
                {c.is_system && <div className={styles.txMeta}>Catégorie système</div>}
              </div>
              {c.is_system ? (
                <span className={styles.badge}>système</span>
              ) : (
...
export default function WalletCategoriesPage() {
  ...
  const reload = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await walletService.listCategories();
  ...
  const submit = async (payload) => {
    if (editing && editing !== 'new') await walletService.updateCategory(editing.id, payload);
    else await walletService.createCategory(payload);
    setEditing(null);
    await reload();
  };
  ...
  const remove = async (cat) => {
    ...
    try { await walletService.deleteCategory(cat.id); await reload(); }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-md">
        <p className="text-muted text-sm" style={{ margin: 0 }}>Catégories système partagées + les vôtres.</p>
